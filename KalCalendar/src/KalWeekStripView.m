...
@implementation KalWeekStripView
...
{    
    if (self = [super initWithFrame:frame]) {
        ...
        [self setDates:logic.daysInSelectedWeek];

        [self selectDateAtIndex:5];
        [self setupViews];
...
- (void)setSelectedDate:(NSDate *)selectedDate
{
    ...
    for (id subview in self.datesScrollView.subviews) {
        if ([subview isKindOfClass:[KalWeekDateTileView class]]) {
            KalWeekDateTileView *dateView = (KalWeekDateTileView *)subview;
...
- (void)updateDatesView
{
    ...
    for (NSDate *date in self.dates) {
        KalWeekDateTileView *dateView = [[KalWeekDateTileView alloc] initWithFrame:CGRectMake(currentItemXPosition, 0, kDIDatepickerItemWidth, self.frame.size.height)];
        dateView.date = date;
...
- (void)updateSelectedDate:(KalWeekDateTileView *)dateView
{
